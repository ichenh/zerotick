...
pub const SUPPORTED: &[&str] = &[
    ...
    "en", "zh-CN", "zh-TW", "ja", "ko", "de", "fr", "es", "pt-BR", "ru", "ar", "hi", "it", "nl", "pl",

    "tr", "vi", "th", "id", "cs", "da", "fi", "nb", "sv", "uk", "he", "ms", "ro", "hu",
...
pub fn tray_tooltip_warning(locale: &str, reason: &str) -> String {
    ...
    let tpl = pick(locale, "tooltip_warning", "ZeroTick — Device fluctuation · {reason}");
...
pub fn notify_transient_title(locale: &str) -> String {
    ...
    pick(locale, "notify_transient", "ZeroTick — Transient disconnect")
...
pub fn notify_disconnect_title(locale: &str) -> String {
    ...
    pick(locale, "notify_disconnect", "ZeroTick — Device disconnected")
...
pub fn format_bluetooth_issue(locale: &str, issue: &BluetoothIssue) -> String {
    ...
    interpolate(
        ...
        &[
            ...
            ("name", issue.name.clone().unwrap_or_else(|| "Unknown".into())),

            (

                "state",

                issue.state.clone().unwrap_or_else(|| "—".into()),

            ),

            ("code", issue.code.map(|c| c.to_string()).unwrap_or_else(|| "—".into())),
