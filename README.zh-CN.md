...
ZeroTick 会在设置页完整列出 37 种受支持的界面语言。英语始终作为离线回退包内置；其余 36 种翻译会作为与应用版本严格匹配的独立 GitHub Release 资源发布，供用户按需安装。首次启动时，ZeroTick 会按照 Windows 首选语言顺序自动匹配并安装最接近的可用翻译。构建门禁会拒绝词条不完整的语言包。
...
| 层 | 实现 |
|----|------|
| 界面 | Vite、Vanilla JavaScript、CSS、1 个离线回退包及独立校验的按需语言包 |
