...
- 网络页工具栏按操作分组：检测与测速位于左侧，DNS 清理与修复位于右侧；网站输入与连接测试独立成行，检测按钮作为主操作突出显示。

### Fixed

- 修复网络页网站标签、输入框与按钮高低不齐的问题；统一控件高度，并在内容区变窄时自动换行，避免操作区域挤成一排。

## [0.2.10] - 2026-09-06

### Added

- 网络面板新增逐网卡诊断，显示连接状态、IPv4/IPv6 地址、网关、DNS、DHCPv4 配置和自动私有地址；网卡信息通过 Windows 原生接口读取。
- 新增指定网站连接测试，分别展示 DNS 解析与 HTTP 响应，区分解析失败、超时、TLS 和连接问题，并明确测试不使用 HTTP 代理。
- 音频修复支持实际重启 Windows Audio，执行前提示播放、通话和录音中断风险；完成后通过实时服务与音频端点证据检查默认播放条件。

### Changed

- 网络修复恢复异常服务并清理 DNS 缓存，清理 DNS 操作也附带目标网站的前后测试；执行记录、测试失败和面板刷新失败分别保留。
- 网络网关探测改用原生 IPv4/IPv6 ICMP，分别检查多网卡候选网关；无回声回复不再直接推断整条网络不可用。

### Fixed

- 修复部分网络查询失败导致其他有效证据丢失的问题；网卡、服务或 VPN/代理状态无法读取时明确显示未知，不再冒充无网卡或未启用。
- 修复并发扫描与服务修复之间的结果混用问题；重叠网络扫描共享结果，修复后重新读取状态，重复修复和音频控制不会相互干扰。
- 修复服务恢复与音频结果的误判：不再将其他进程发起的服务启动计作本次修复，不再将未知音量或静音状态当作正常；通用服务修复也不再清除未经重新诊断的托盘警告。

## [0.2.9] - 2026-07-29

### Changed

- 诊断属性列表统一为左侧标签、右侧结果的稳定布局；网关地址与可达状态改为独立字段，代理、VPN 和连接诊断等右侧多项值不再显示无意义的项目圆点。
- 长文本、设备标识和多语言状态值使用自然断词与安全换行，避免按任意字符强制拆分。

### Fixed

- 修复高级模式显隐规则覆盖组件原有布局的问题，网络测速、默认网关、网卡统计、蓝牙服务与适配器、占用进程路径和高级设置区域不再退化为挤在一起的普通文本流。
