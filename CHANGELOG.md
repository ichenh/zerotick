...
- 管理员自启改为按当前用户创建 Windows 最高权限登录任务：首次配置只确认一次 UAC，后续登录静默启动，不再每次开机重复确认。
- 蓝屏追溯在没有可用小型转储时继续读取可信的 Windows BugCheck 事件，并明确区分事件级停止代码分类与完整转储分析。

### Changed

- 网络诊断通过 Windows Network List Manager/NCSI 分别判断网卡存在、实时链路与互联网访问；断开、仅局域网、未知证据和网关/服务异常不再混为“网络正常”。
- 可移动存储安全弹出改由 Windows PnP 安全移除结果主导；针对带只读虚拟光驱的复合移动硬盘，数据卷已安全刷新并卸载时会如实报告部分成功，不冒充整个设备已消失。
- 37 种内置语言补齐管理员自启、复合存储安全弹出、格式化后验验证、蓝牙操作验证和无转储蓝屏证据等安全关键文案。

### Fixed

- 蓝牙设备移除和重启后重新读取目标设备；只有设备确实消失或恢复为正常状态时才报告成功。
- USB 格式化完成后重新读取文件系统与卷标；Windows 命令退出成功但目标状态无法确认时不再显示“格式化完成”。
- 修复 USB 后台深度检测失败后界面长期停留在“正在读取”、蓝屏修复存在失败项却仍弹成功提示，以及管理员自启配置失败未在应用内说明的问题。
- 收紧可移动存储弹出目标、设备组件和可能占用程序的证据表述，避免把命令行路径线索当作已确认句柄或把 Windows 仍保留的复合设备节点当作弹出失败。
